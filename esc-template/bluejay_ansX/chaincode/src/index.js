     ...
     async analysis(ctx, params) {
        ...
        if(frmDates.length > 0){
            ...
            for(let k=0; k<frmDates.length; k++){
                ...
                console.log('data[0].Record.responses');
                console.log(data[0].Record.responses);
                ...
                /*let metricsReduced = metricValuesAux.reduce((acc, curr) => {
                    Object.entries(curr).forEach(([key, value]) => {
                        ...
                        console.log(key);
                        console.log(value);
                        console.log(value.evidences);
                        acc[key] = acc[key] || [];
                        for (let evIndex = 0; evIndex < value.evidences.length; evIndex++) {
                            acc[key].push(parseInt(value.evidences[evIndex].value));
                ...
                /*for (let key in metricsReduced) {
                    metricValues[key] = {}
                    metricValues[key].value = metricsReduced[key].reduce((a, b) => a + b, 0) / metricsReduced[key].length;
                    metricValues[key].evidences = metricValuesAux.reduce((acc, curr) => {
                        Object.entries(curr).forEach(([key2, value2]) => {
                            acc[key2] = acc[key2] || [];
                            acc[key2].push(value2.evidences);
                        });
                        return acc;
                    } , {});
                }*/
                ...
                console.log(metricValues);
                //console.log('??');
                //console.log(console.log(data[0].Record));
                //console.log('timedScopes');
                //console.log(data[0].Record.responses[0].agreement);
                ...
                console.log(timedScopes);
                    ...
                    //console.log('agreement.terms.guarantees');
                    //console.log(agreement.terms.guarantees);
