...
let config = {
  ...
  resultsPath: "./experiments/experiments_results/4/bluejay_ansX/",
  identityName: "admin",
  channelName: "escchannel",
  chaincodeName: "bluejay_ansX",
...
async function hookData(metricQueries, agreement){
  ...
  let requests = metricQueries.map( metricQuery => {
    ...
    return new Promise(async (resolve) => {
      if(metricQuery.collector.type === 'PPINOT-V1'){
        console.log('ESC/OTI_GC_ANSX - hookData - PPINOT-V1');
        process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
        //console.log('collector.infrastructurePath');
        //console.log(collector.infrastructurePath);
        //console.log(collector.endpoint + '/' + collector.name.replace(/^\//, '') + '?' + encodeURI(urlParams));
        ...
        console.log('ESC/OTI_GC_ANSX - hookData - POST-GET-V1');
        console.log('collector.infrastructurePath');
        console.log(collector.infrastructurePath);
        ...
        console.log('monthMetrics - postgetv1 - pre try')
        console.log(monthMetrics)
        try {
          // Check if computer response is correct
          if (monthMetrics && Array.isArray(monthMetrics)) {
            console.log('monthMetrics - postgetv1')
              console.log(monthMetrics)
            // For each state returned by computer map the scope
            monthMetrics.forEach(function (metricState) {
              console.log('metricState - postgetv1')
  ...
  return Promise.all(requests).then(() => {
    console.log('ESC/OTI_GC_ANSX - hookData - promise.all');
...
function containsObject (obj, array) {
  console.log('ESC/OTI_GC_ANSX - containsObject');
...
function computerToRegistryParser (computerScope, mapping) {
  console.log('ESC/OTI_GC_ANSX - computerToRegistryParser');
  const mappedScope = {};
  // reversing mapping
...
function getComputationV2 (infrastructurePath, computationURL, ttl) {
  console.log('ESC/OTI_GC_ANSX - getComputationV2');
  console.log('computationURL')
  console.log(computationURL)
  console.log('infrastructurePath')
  console.log(infrastructurePath)
    ...
    try {
      ...
      setTimeout(async () => {
        governify.infrastructure.getService(infrastructurePath).get(computationURL).then(response => {
          ...
          console.log('getComputationV2 err')
          console.log(err)
      ...
      console.log('getComputationV2 try err')
      console.log(err)
      reject(err);
    }
  });
...
async function intervalHarvester(frequency, metricQueries, agreement) {
  console.log('ESC/OTI_GC_ANSX - intervalHarvester');

  if(config.elasticityMode == "harvestFrequency"){
    console.log('ESC/OTI_GC_ANSX - intervalHarvester - harvestFrequency');
    ESC.frequencyChanged(config.chaincodeName);
    interval = setInterval(() => {
  
      ESC.getNewFrequency(config.chaincodeName).then(async (res) =>{
        console.log('ESC/OTI_GC_ANSX - intervalHarvester - harvestFrequency - getNewFrequency');
  
        if(res.change){
          console.log('ESC/OTI_GC_ANSX - intervalHarvester - harvestFrequency - getNewFrequency - change');
          ...
          console.log('ESC/OTI_GC_ANSX - intervalHarvester - harvestFrequency - getNewFrequency - no change');
    ...
    console.log('ESC/OTI_GC_ANSX - intervalHarvester - no harvestFrequency');
    interval = setInterval(async () => {
    
      let newData = await hookData(metricQueries, agreement);
      //console.log('newData - intervalHarvester');
      //console.log(newData);
...
function start(metricQueries, agreement){
  console.log('ESC/OTI_GC_ANSX - start');
  ...
  ESC.connect(config.chaincodeName).then(async () =>{
    console.log('ESC/OTI_GC_ANSX - start - connect');

    intervalColdStart = setInterval(async () => {
      //console.log('ESC/OTI_GC_ANSX - start - intervalColdStart');
